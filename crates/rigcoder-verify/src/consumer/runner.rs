...
pub(crate) async fn run(args: impl IntoIterator<Item = String>) -> Result<(), Error> {
    ...
    artifacts::write(
        &artifacts::root().join(".rigcoder-verify/report.json"),
        &report,
    )?;
    eprintln!(
        "rigcoder-verify {}: {passed}/{} passed, {} failed; {} transport requests. Report: .rigcoder-verify/report.json",
        invocation.command,
        selected.len(),
        selected.len() - passed,
        total_requests
    ...
    if failed {
        return Err(Error::Invariant(
            "required cases failed; see .rigcoder-verify/report.json".into(),
        ));
    }
    Ok(())
}
