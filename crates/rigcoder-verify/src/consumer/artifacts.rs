...
pub(crate) fn candidate(case: &Case) -> Result<PathBuf, Error> {
    let stamp = std::time::SystemTime::now()
        ...
        .as_nanos();
    let path = root().join(".rigcoder-verify/candidates").join(format!(
        "{}-{stamp}-{}",
        case.id,
...
pub(crate) fn compare_from(
    ...
    for (name, actual) in parts(evidence)? {
        ...
        if let Some(first) = diffs.first() {
            ...
            return Err(Error::Invariant(format!(
                "{} {name}: {} semantic differences; first at {}; failure evidence {}; reproduce: cargo run -p rigcoder-verify -- verify --case {}",
                case.id,
                diffs.len(),
                first.pointer,
                bundle.display(),
...
pub(crate) fn census() -> Result<(), Error> {
    ...
    let actual = std::fs::read_dir(root().join("fixtures/verify"))?
