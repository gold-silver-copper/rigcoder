...
```sh
cargo run -p rigcoder-verify -- plan
cargo run -p rigcoder-verify -- verify
cargo run -p rigcoder-verify -- verify --matrix provider
cargo run -p rigcoder-verify -- verify --case stream-single-events
cargo run -p rigcoder-verify -- replay --case anthropic-stream
cargo run -p rigcoder-verify -- resume --case anthropic-stream --cut after-write
cargo test -p rigcoder-verify --test consumer
```

CI runs the offline verifier on Linux and macOS without provider credentials,
including the full `verify` command and the product/verifier tests. Linux uses
`.github/scripts/setup-verifier-sandbox.sh` to provision bubblewrap and probe
user namespaces; macOS requires `sandbox-exec`. Missing isolation fails the job.
The jobs retain `.rigcoder-verify/report.json` for diagnosis.
...
```sh
cargo run -p rigcoder-verify -- record --case anthropic-unary
```

The command prints its candidate path. Use that path in the following steps:

```sh
cargo run -p rigcoder-verify -- derive --case anthropic-unary --candidate .rigcoder-verify/candidates/REPLACE_WITH_RECORDED_CASE
cargo run -p rigcoder-verify -- replay --case anthropic-unary --candidate .rigcoder-verify/candidates/REPLACE_WITH_DERIVED_CASE
cargo run -p rigcoder-verify -- promote --case anthropic-unary --candidate .rigcoder-verify/candidates/REPLACE_WITH_DERIVED_CASE
```
...
```sh
cargo run -p rigcoder-verify -- resume --case zero-progress-stream --cut before-first-delivery
cargo run -p rigcoder-verify -- verify --case replay-guarantees,external-write-outcome-lost
```
...
1. Run `plan`, select the relevant matrix and verify it offline. Reports and
   failure bundles are under ignored `.rigcoder-verify/`; preserve the failing
...
```sh
cargo run -p rigcoder-verify -- plan --matrix repository-repair
cargo run -p rigcoder-verify -- verify --matrix repository-repair
```
...
```sh
failure_bundle_path=.rigcoder-verify/candidates/REPLACE_WITH_FAILED_CASE/failure.json
...
if len(argv) < 6 or argv[:5] != ["cargo", "run", "-p", "rigcoder-verify", "--"]:
    raise SystemExit("Unexpected reproduction command")
if argv[5] not in ("verify", "replay", "resume", "derive"):
...
```sh
cargo test -p rigcoder-verify --test consumer failure_bundle_preserves_effective_config_provider_digest_and_runtime_link
cargo test -p rigcoder-verify --test consumer consumer::diagnostics
cargo test -p rigcoder-verify --test consumer failed_capture_identity_uses_its_own_partial_traffic
```
...
`.rigcoder-verify/repair-loop/evidence.json`; no temporary fault or replacement
...
`crates/rigcoder-verify/src/registry.rs`. It sets a grouped delivery size in the existing
...
```sh
cargo run -p rigcoder-verify -- plan --case repair-stream-groups
cargo run -p rigcoder-verify -- verify --case repair-stream-groups
cargo test -p rigcoder-verify --test consumer consumer_fixtures_match_the_executable_registry_and_producer_hashes
```
...
```sh
cargo run -p rigcoder-verify -- record --case repair-anthropic-unary
cargo run -p rigcoder-verify -- derive --case repair-anthropic-unary --candidate .rigcoder-verify/candidates/REPLACE_WITH_CAPTURE
cargo run -p rigcoder-verify -- verify --case repair-anthropic-unary --candidate .rigcoder-verify/candidates/REPLACE_WITH_DERIVATION
cargo run -p rigcoder-verify -- resume --case repair-anthropic-unary --candidate .rigcoder-verify/candidates/REPLACE_WITH_DERIVATION --cut after-write
```
...
The project copied from `harness/repair-project` deliberately stays
