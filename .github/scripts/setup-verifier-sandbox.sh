#!/usr/bin/env bash
# Provision the Linux backend used by the rigcoder repository-repair verification.
set -euo pipefail
...
if [[ -r /proc/sys/kernel/apparmor_restrict_unprivileged_userns ]] &&
  ...
  sudo tee /etc/apparmor.d/rigcoder-verifier-bwrap >/dev/null <<'PROFILE'
abi <abi/4.0>,
include <tunables/global>
profile rigcoder-verifier-bwrap /usr/bin/bwrap flags=(unconfined) {
  userns,
}
PROFILE
  sudo apparmor_parser -r /etc/apparmor.d/rigcoder-verifier-bwrap
fi
